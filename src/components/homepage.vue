      ...
      >
        ...
        <div class="co-buttons-update" :data-id=m.id v-show="buttonActiveChange">
          <button
              ...
              :disabled="m.handel_status === 1"
              :class="{changColorGrey: m.handel_status === 1}"
              class="co-button-update button-handle"
          >已处理</button>
          <button
              v-else
              @click="actionClickHandle(i)"
        ...
        <div class="co-handle" v-show="HandleYes">
          ...
          <button class="co-pan-handle co-right changBackGround " @click="actionHandleYes">确定</button>
        </div>
      </Card>
    ...
    goTouchEnd(e){
      ...
      if (this.distance > 30) {
        ...
        if (this.ifActive === true) {
          if (this.pageOption.total > this.userMessage.length) {
            this.getData(t)
          }
        } else {
          if (this.optionTotal.total > this.optionTotal.len) {
      ...
      apis.report_get_data(params).then(res => {
        ...
        if (errCode === 0) {
          ...
          for (let i = 0; i < data.records.length; i++) {
            this.cloneContentType.push(data.records[i].content_type)
          }
          // console.log('content_type', this.cloneContentType)
        }
      }).catch()
    },
    getVerifyValue(data) {
      const { total, len } = data
      this.optionTotal.total = total
      this.optionTotal.len = len
      // console.log('this.optionTotal', this.optionTotal)
    },
